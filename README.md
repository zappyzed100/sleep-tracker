...
- **ローカル自動バックアップ（PC版）** — 1日ごとに `sleep_events.txt`/`sleep_manual.txt` を `data/backups/` へ自動保存（直近10世代を保持、それより古い世代は自動削除）
- **監視中断 / 再開** — UI から手動で睡眠検知を一時停止
- **外出除外** — スマートフォン / タブレットの位置情報や操作を検知して外出中の誤記録を自動除外
- **Android ビューアー** — タブレットやスマートフォンで睡眠ログを確認（Drive 経由で PC と同期、バックグラウンドでも 15 分ごとに自動送信）
...
② PC アプリ（Windows）           ③ Android アプリ（任意ビューアー）
   ・睡眠自動検知                    ・睡眠グラフ閲覧
   ・Drive バックアップ              ・Drive からデータ取得・バックグラウンド送信
      ↕ スプレッドシート
...
> 端末の生存確認（DEVICE_ON）は Android ビューアーアプリが起動中・バックグラウンド問わず自動送信するため、MacroDroid 側でのマクロは不要です。
...
1. スプレッドシートからモバイルイベント（外出・帰宅・DEVICE_ON）を取得してローカルに統合
...
Drive への同期とは別に、1日ごとに `sleep_events.txt`/`sleep_manual.txt` のスナップショットを `data/backups/` に保存します。直近10世代（約10日分）だけを保持し、それより古いものは自動削除されます。Drive 同期が止まっていた場合のローカル側の保険用途です。

**データ管理（設定タブ →「データ管理」）**

| ボタン | 効果 |
|-------|------|
| ローカルの全データ削除 | PC 上の `sleep_events.txt`/`sleep_manual.txt` だけを空にする（Drive・スプレッドシートは残る） |
...
- データは Drive から自動取得（アプリ起動時・前面復帰時・15 分ごとにバックグラウンドでも自動）
...
│   │   └── backups/             # 日次ローカル自動バックアップ（直近10世代、PC版のみ）
...
| `DEVICE_ON` | タブレット | 端末の生存確認（起動・前面復帰・15分ごとのバックグラウンド送信） |
