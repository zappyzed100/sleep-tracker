...
スマートフォンから届く `LEAVE_HOME` / `ARRIVE_HOME`（GPS由来）が `OUT_START` / `OUT_END` として記録されます。また、タブレットの画面が実際にONだった区間（`UsageStatsManager` の `SCREEN_INTERACTIVE`/`SCREEN_NON_INTERACTIVE` 由来）が `SCREEN_ON_START` / `SCREEN_ON_END` として記録されます。いずれも「起きていた証拠」として全く同じ扱いで、この期間中の PC アイドルは睡眠から除外されます。

> 以前は「どのアプリがフォアグラウンドか」（`MOVE_TO_FOREGROUND`/`MOVE_TO_BACKGROUND`）を見ていましたが、画面が自動消灯した後も機種によっては直前のアプリが「まだフォアグラウンド扱い」のまま残り続け、画面OFFで何時間も放置していた時間まで利用区間として記録されてしまうバグがありました。画面が実際にON/OFFになった実イベントを直接見ることでこれを解消しています。短すぎる点灯（1分未満）はノイズとして無視し、近接する点灯は1回の利用として統合してから判定します。

PC 操作（`IDLE_START` など）またはタブレットの画面ON（`SCREEN_ON_START`）が検知された場合、外出中でも自動的に `IN_HOUSE` イベントを記録して外出状態を解除します。
...
| `SCREEN_ON_START` / `SCREEN_ON_END` | タブレット | 画面が実際にONだった区間（`UsageStatsManager`の`SCREEN_INTERACTIVE`由来）。睡眠判定から除外され、在宅解除にも使われる |
