...
PC 操作（`IDLE_START` など）またはタブレット操作（`DEVICE_ON` / `APP_USAGE_START`）が検知された場合、外出中でも自動的に `IN_HOUSE` イベントを記録して外出状態を解除します。
...
| `DEVICE_ON` | タブレット | アプリを開いた（起動・前面復帰時のみ。睡眠判定には使わず在宅解除専用） |
| `APP_USAGE_START` / `APP_USAGE_END` | タブレット | 実際にアプリを使っていた区間（`UsageStatsManager`由来）。睡眠判定から除外される |
