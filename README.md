...
> **移行中**：本 README は現行の Tauri 版（`src_tauri/`）を対象としています。
> Rust + Slint で PC・Android 共通コードベースへ書き換え中の後継実装が `src_slint/` にあります
> （詳細は [`src_slint/README.md`](src_slint/README.md) 参照）。まだ機能が揃っていないため、
> 現時点では `src_tauri/` が動作保証のある実装です。
