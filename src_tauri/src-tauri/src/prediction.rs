...
fn make_features(bh: f64, awake_h: f64, wd: usize) -> Vec<f64> {
    let mut f = vec![
        (bh * 2.0 * PI / 24.0).sin(),
        (bh * 2.0 * PI / 24.0).cos(),
        awake_h,
...
pub fn find_optimal(sessions: &[Session], now: &str) -> Option<OptimalResult> {
    ...
    // Every 30 min from now+30 to now+12h (24 candidates)
    let candidates: Vec<(f64, usize, i64, i64)> = (1i64..=24)
        .map(|slot| {
    ...
    let feat_list: Vec<Vec<f64>> = candidates
        .iter()
        .map(|(bh, wd, _, _)| make_features(*bh, awake_h, *wd))
        .collect();
    ...
    let durations: Vec<f64> = if sessions.len() >= 10 {
        let x_rows: Vec<Vec<f64>> = sessions
            ...
            .map(|(i, s)| {
                ...
                make_features(bed_hour(&s.start), aw, weekday_idx(&s.start))
            })
            .collect();
        ...
        match RandomForestRegressor::fit(&x_mat, &y, params).and_then(|m| m.predict(&x_q)) {
            ...
            Err(_) => candidates.iter().map(|(bh, _, _, _)| heuristic(sessions, *bh).0).collect(),
        }
    } else {
        candidates.iter().map(|(bh, _, _, _)| heuristic(sessions, *bh).0).collect()
    };
    ...
    durations
        ...
        .map(|(i, &min_dur)| {
            let (_, _, h, m) = candidates[i];
            OptimalResult {
                best_bed_time: format!("{:02}:{:02}", h, m),
...
pub fn predict(sessions: &[Session], now: &str) -> PredictionResult {
    ...
    let awake_h = sessions
        .last()
        .map(|s| awake_between(&s.end, now))
        .unwrap_or(16.0);
    ...
    let x_mat = DenseMatrix::from_2d_vec(&x_rows);
    let x_q = DenseMatrix::from_2d_vec(&vec![make_features(bh, awake_h, wd)]);
    ...
    match RandomForestRegressor::fit(&x_mat, &y, params) {
        Ok(model) => match model.predict(&x_q) {
            Ok(preds) => PredictionResult {
                ...
                method: format!("Machine Learning (awake {:.1}h)", awake_h),
                awake_hours: awake_h,
            },
            Err(_) => {
                let (dur, method) = heuristic(sessions, bh);
