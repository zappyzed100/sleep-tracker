//! android_usage.rs — 「睡眠判定に使うアプリ」のKotlin↔Rust JNIブリッジ
//!
...
//!
//! 依存 : crate::core::events, crate::platform::android_bg, jni
    ...
    unowned_env
        .with_env(|env| -> jni::errors::Result<()> {
            ...
            if let Err(e) = events::record_usage_package_seen(&pkg, &lbl) {
                eprintln!("[app] android_usage: ERROR record_usage_package_seen: {}", e);
            }
            Ok(())
        })
...
// android_bg::refresh_ui()をキックし、新規検知パッケージが設定画面に反映
...
pub extern "system" fn Java_com_sleeptracker_app_UsageReporter_nativeReportUsageScanComplete<'caller>(
    ...
    crate::platform::android_bg::refresh_ui();
}
