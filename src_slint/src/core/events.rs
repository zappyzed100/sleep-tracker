...
// 「睡眠判定に使うアプリ」機能。Android側のUsageStatsManagerはどのアプリがフォア
// グラウンドにあったかを区別せず検出するため、自アプリ自身やランチャーが一瞬
// フォアグラウンドに来ただけでも「使用した」と誤判定されることがあった
// （UsageReporter.kt参照）。検知したパッケージ名をUSAGE_APP_SEEN:{pkg}として
// sleep_events.txtに記録し、設定画面（PC/Android両方）でON/OFFを選べるように
// する。ON/OFFの変更はUSAGE_APP_ALLOWED:{pkg}/USAGE_APP_DENIED:{pkg}マーカーとして
// 記録し、DAY_EXCLUDED/DAY_INCLUDEDと同じ「最後のマーカーが勝つ」方式で解決する。
// このファイル自体が既存のDrive同期・HARD_RESET伝播にそのまま乗るため、
// PC/Android間の追加の同期経路は不要（cloud::merge_into_local参照）。
...
fn usage_packages_from_content(content: &str) -> Vec<UsagePackageEntry> {
...
// （compact_data参照）。
fn extract_preserved_metadata_lines(raw: &str) -> Vec<String> {
    raw.lines()
